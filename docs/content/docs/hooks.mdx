...
```ts
const { submitBug, submitting, error, lastResponse, reset } = useBugReport()
```
...
| --- | --- | --- |
| `submitBug` | `(payload: BugReportPayload) => Promise<SubmitBugReportResponse>` | Submit a report. Rejects on failure |
